...
async def assert_automation_page():
    ...
    assert 'data.message || "Не удалось загрузить состояние A3"' in html
    assert 'data.message || "Не удалось загрузить аналитику автоматизации"' in html
    assert 'data.message || "Не удалось загрузить операционные инсайты"' in html
    assert 'data.message || "Не удалось запустить самовосстановление"' in html
    assert 'data.message || "Не удалось загрузить историю восстановления"' in html
    ...
    assert "<th>Номер</th>" in rule_detail_html
    assert "<th>ID</th>" not in rule_detail_html
    ...
    assert workflow_graph["rule"]["status_label"] == "Активно"
    ...
    assert all(node.get("status_label") for node in workflow_graph["nodes"])
    assert any(
    ...
    assert all(action.get("status_label") for action in workflow_graph["actions"])
    assert any(
    ...
    assert "loaded_label" in workflow_timeline["timeline"]
    assert workflow_timeline["timeline"]["loaded_label"].startswith("Показано:")
    assert "load_state_label" in workflow_timeline["timeline"]
    assert workflow_timeline["timeline"]["load_state_label"] in {
        "Есть ещё события",
    ...
    if workflow_timeline["timeline"]["sessions"]:
        ...
        assert session["execution_state_label"] in {
            "В работе",
            "Завершено",
            "Нужно внимание",
            "Проблема",
    ...
    assert filtered_workflow_timeline["timeline"]["status_filter_label"] == "Ошибки"
    assert all(
        item["status"] == "failed"
...
async def assert_automation_runner(task):
    ...
    assert "Фильтр по правилу:" in rule_events_html
    assert f"SLA runner rule (#{event['rule_id']})" in rule_events_html
...
async def assert_a3_workflow_center():
    ...
    assert "attentionLabel ? \" · \" + attentionLabel : \"\"" in body
    ...
    assert 'data.message || "Не удалось загрузить историю"' in body
    assert "setWorkflowActionStatus(message, \"error\")" in body
